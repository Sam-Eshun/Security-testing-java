...
$trimmedSelectclass=intval($_POST['selectclass']);
 if($_POST['editgrade'] == 1)
 {
  ...
  else
  {
   ...
   $query = mysql_query('SELECT semesterid,termid FROM courses WHERE courseid = '.intval($_POST["selectclass"]));
   $ids = mysql_fetch_row($query);

   $query = mysql_query("INSERT INTO grades VALUES('', '$_POST[assignment]', '$trimmedSelectclass', '$ids[0]', '$ids[1]', '$_POST[student]', '$_POST[points]', '$_POST[comment]', '".converttodb($_POST['gradedate'])."', '".($_POST['late']==1 ? "1" : "0")."')")
  ...
  $query = mysql_query("SELECT semesterid FROM courses WHERE courseid = $trimmedSelectclass");
  ...
  {
   if($wasdate < $middate)
	$query = mysql_query("UPDATE registrations SET q1currpoints=(q1currpoints + $_POST[points] - $_POST[wasgrade]), currentpoints=(currentpoints + $_POST[points] - $_POST[wasgrade]) WHERE courseid = $trimmedSelectclass AND studentid = $_POST[student]");
   else
	$query = mysql_query("UPDATE registrations SET q2currpoints=(q2currpoints - $_POST[wasgrade]), q1currpoints=(q1currpoints + $_POST[points]), currentpoints=(currentpoints + $_POST[points] - $_POST[wasgrade]) WHERE courseid = $trimmedSelectclass AND studentid = $_POST[student]");
  }
  else
  {
   if($wasdate < $middate)
	$query = mysql_query("UPDATE registrations SET q1currpoints=(q1currpoints - $_POST[wasgrade]), q2currpoints=(q2currpoints + $_POST[points]), currentpoints=(currentpoints + $_POST[points] - $_POST[wasgrade]) WHERE courseid = $trimmedSelectclass AND studentid = $_POST[student]");
   else
	$query = mysql_query("UPDATE registrations SET q2currpoints=(q2currpoints + $_POST[points] - $_POST[wasgrade]), currentpoints=(currentpoints + $_POST[points] - $_POST[wasgrade]) WHERE courseid = $trimmedSelectclass AND studentid = $_POST[student]");
  }
 }
 ...
 if($_POST["deletegrade"] == 1)
 {
  ...
  for($i=0; $i<sizeof($delete); $i++)
  {
   ...
   {
	$query = mysql_query("UPDATE registrations SET q2currpoints=(q2currpoints - $id[1]), currentpoints=(currentpoints - $id[1]) WHERE studentid = $id[3] AND courseid = $trimmedSelectclass");
   }
  }
 }
...
print("<script language='JavaScript'>
 ...
 $query = mysql_query("SELECT * FROM assignments WHERE courseid = '$trimmedSelectclass'");
 ...
 $query = mysql_query("SELECT assignmentid,title FROM assignments WHERE courseid = '$trimmedSelectclass'")
...
print("  </select>
  ...
  $query = mysql_query("SELECT DISTINCT s.studentid, s.fname, s.lname FROM students s, registrations r WHERE s.studentid = r.studentid AND r.courseid = $trimmedSelectclass ORDER BY UPPER(s.lname) ASC")
  ...
  while($student = mysql_fetch_row($query))
  {
   ...
   $q = mysql_query("SELECT gradeid, points, comment, submitdate, islate, studentid, comment FROM grades WHERE assignmentid = '$_POST[assignment]' AND courseid = '$trimmedSelectclass' AND studentid = '$student[0]'")
   ...
   // Calculate and display the letter grade //
	$q = mysql_query("SELECT aperc,bperc,cperc,dperc,fperc FROM courses WHERE courseid = $trimmedSelectclass") or die("ManageGrades.php: Unable to get the grade percentages - ".mysql_error());
...
print("  </table>
  ...
  <input type='hidden' name='selectclass' value='$trimmedSelectclass' />
